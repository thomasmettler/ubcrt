/**
 *  @file   larpandora/LArPandoraEventBuilding/NeutrinoCRTmatcher_module.cc
 *
...
class NeutrinoCRTmatcher : public art::EDAnalyzer
{
public:
    explicit NeutrinoCRTmatcher(fhicl::ParameterSet const & pset);
...
DEFINE_ART_MODULE(NeutrinoCRTmatcher)
...
NeutrinoCRTmatcher::NeutrinoCRTmatcher(fhicl::ParameterSet const &pset) :
  EDAnalyzer(pset),
    ...
    m_pandoraTag(art::InputTag(m_inputProducerLabel))
{
    ...
    use_ts1_ = pset.get<double>("use_ts1",0);
...
void NeutrinoCRTmatcher::analyze(art::Event const &evt)
{
  reset_tree();
  ...
  //get t0 association from tcp tracks
  ...
  if(run_MC > 1){
    ...
    evt.getByLabel(m_trackProducerLabel, rawHandle_Track);
    //TrackCollection.clear();
    std::vector<recob::Track> const& TrackCollection_tmp(*rawHandle_Track);
    if(verbose_!=0)std::cout << "MC track collectionSize: " << TrackCollection_tmp.size() << std::endl;
    //TrackCollection.insert(TrackCollection.end(), TrackCollection_tmp.begin(), TrackCollection_tmp.end());
  
  
    for(std::vector<int>::size_type i = 0; i != TrackCollection_tmp.size(); i++) {    //start loop over tpc tracks
      const std::vector<const anab::T0*>& T0crt = trk_T0_assn_v.at(i);
      if(T0crt.size()!=0){
        if(verbose_!=0){
          std::cout << "################################################" << std::endl;
          std::cout << "Found T0 object from crthit - track association:" << std::endl;
  ...
  if(beam_flash_index!=-1 && beam_crthit_index!=-1){
    ...
    if( abs(crtt0_time - crt_time) < 100 ){
      if(verbose_!=0){
        std::cout << "################################################" << std::endl;
...
void NeutrinoCRTmatcher::CollectPFParticles(const art::Event &evt, PFParticleToMetadata &particlesToMetadata, PFParticleVector &particles) const
{
    ...
    for (unsigned int i = 0; i < pfParticleHandle->size(); ++i)
    {
        ...
        if (metadata.size() != 1) 
            throw cet::exception("LArPandora") << " NeutrinoCRTmatcher::CollectPFParticles -- Found a PFParticle without exactly 1 metadata associated." << std::endl;

        if (!particlesToMetadata.insert(PFParticleToMetadata::value_type(part, metadata.front())).second)
            throw cet::exception("NeutrinoCRTmatcher") << "Repeated PFParticles" << std::endl;
    }
}

//------------------------------------------------------------------------------------------------------------------------------------------

void NeutrinoCRTmatcher::BuildPFParticleMap(const PFParticleToMetadata &particlesToMetadata, PFParticleMap &particleMap) const
{
...
void NeutrinoCRTmatcher::CollectClearCosmicRays(const PFParticleVector &allParticles, const PFParticleToMetadata &particlesToMetadata, const PFParticleMap &particleMap, PFParticleVector &clearCosmics) const
{
    ...
    for (const auto &part : allParticles)
    {
        ...
        if (parentIt == particlesToMetadata.end())
            throw cet::exception("NeutrinoCRTmatcher") << "Found PFParticle without metadata" << std::endl;
...
void NeutrinoCRTmatcher::CollectSlices(const PFParticleVector &allParticles, const PFParticleToMetadata &particlesToMetadata, const PFParticleMap &particleMap, SliceVector &slices) const
{
    ...
    for (unsigned int sliceId = 1; sliceId <= nuScores.size(); sliceId++)
    {
        ...
        if (nuScoresIter == nuScores.end()){}
            //throw cet::exception("NeutrinoCRTmatcher") << "Scrambled slice information - can't find nuScore with id = " << sliceId << std::endl;
...
float NeutrinoCRTmatcher::GetMetadataValue(const art::Ptr<larpandoraobj::PFParticleMetadata> &metadata, const std::string &key) const
{
    ...
    if (it == propertiesMap.end())
        throw cet::exception("NeutrinoCRTmatcher") << "No key \"" << key << "\" found in metadata properties map" << std::endl;
...
void NeutrinoCRTmatcher::CollectConsolidatedParticles(const PFParticleVector &allParticles, const PFParticleVector &clearCosmics, const SliceVector &slices, PFParticleVector &consolidatedParticles) const
{
...
//------------------------------------------------------------------------------------------------------
void NeutrinoCRTmatcher::initialize_tmyevent()
{
...
void NeutrinoCRTmatcher::reset_tree()
{
  fAbsTimFla = -1;
  flash_PE = -1;
  flash_y = -1;
...
void NeutrinoCRTmatcher::beginJob()
{
  // Implementation of optional member function here.
  std::cout << "Starting Neutrino CRT matcher module" << std::endl;
  //initialize_tpandora();
  initialize_tmyevent();
...
void NeutrinoCRTmatcher::endJob()
{
  // Implementation of optional member function here.
